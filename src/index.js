...
// 이메일 유효성 검사 함수
function emailCheck() {
  const email = document.getElementsByClassName("product-billing-addInfo_email")
    .value;
...
const templates = {
  ...
  productBillingInfo: document.querySelector("#product-billing-addInfo")
    .content,
...
async function postProductPage() {
  ...
  fragment
    .querySelector(".post-product-form__back-btn")
...
async function editFormPage(productId) {
  ...
  fragment
    .querySelector(".edit-form__back-btn")
    .addEventListener("click", e => {
      e.preventDefault();
...
async function loginPage() {
  ...
  formEl.addEventListener("submit", async e => {
    ...
    e.preventDefault();
    const res = await productAPI.post("users/login", payload).then(
      async response => {
...
async function joinPage() {
  ...
  formEl.addEventListener("submit", async e => {
    ...
    e.preventDefault();
    const res = await productAPI.post("/users/register/", payload).then(
      response => {
        alert(`가입해주셔서 감사합니다`);
        loginPage();
      },
      error => {
        if (error.response.status == 400) {
          ...
          pEl.textContent =
            "서버쪽 문제가 발생하였습니다. 잠시 후에 다시 시도해주십시요";
        }
      }
    );
  });
...
async function myPages() {
  ...
  if (res.data[0] == null) {
    ...
    fragment
      .querySelector(".myPage__back-btn")
    ...
    fragment
      .querySelector(".myPage__logout-btn")
      .addEventListener("click", e => {
        logout();
    ...
    res.data.forEach(item => {
      ...
      itemFragment.querySelector(".myPage-item__title").textContent =
        item.title;
      itemFragment.querySelector(".myPage-item__count").textContent =
        item.amount;
      itemFragment.querySelector(".myPage-item__name").textContent = item.name;
      itemFragment.querySelector(".myPage-item__tel").textContent =
        item.cellPhone;
      itemFragment.querySelector(".myPage-item__email").textContent =
        item.email;
      itemFragment.querySelector(".myPage-item__requestText").textContent =
        item.requestText;
...
async function cartPage() {
  ...
  if (res.data[0] == null) {
    ...
    let sumCost;
    fragment
      .querySelector(".cart-product-item__main-btn")
      .addEventListener("click", e => {
        indexPage();
      });
    fragment
      .querySelector(".cart-product-item__back-btn")
      .addEventListener("click", e => {
        productPage(res.data[res.data.length - 1].productId);
      });
    fragment
      .querySelector(".product__logout-btn")
      .addEventListener("click", e => {
        logout();
        alert("감사합니다! 즐거운 하루 되십시요!");
        indexPage();
    ...
    fragment
      .querySelector(".cart-product__order")
      .addEventListener("click", async e => {
        // 장바구니 -> 결제페이지로 넘기는 것
        orderPage();
      });
    ...
    res.data.forEach(cart => {
      ...
      cartFragment.querySelector(".cart-product-item__img").src =
        cart.product.imgUrl;
      cartFragment.querySelector(".cart-product-item__title").textContent =
        cart.product.title;
      cartFragment.querySelector(".cart-product-item__count").textContent =
        cart.amount;
      cartFragment.querySelector(
          ".cart-product-item__product-cost"
        ).textContent =
        commas(cart.product.cost * cart.amount) + "원";
      sumCost = cart.product.cost * cart.amount;
      sum.push(sumCost);
      sumCost = 0;
      cartFragment
        .querySelector(".cart-product-item__delete-btn")
        .addEventListener("click", async e => {
...
async function orderPage() {
  ...
  let sumCost;
  fragment
    .querySelector(".product-billing__main-btn")
    .addEventListener("click", e => {
      indexPage();
    });
  fragment
    .querySelector(".product-billing__back-btn")
    .addEventListener("click", e => {
      productPage(res.data[res.data.length - 1].productId);
    });
  fragment
    .querySelector(".product-billing__logout-btn")
    .addEventListener("click", e => {
      logout();
      alert("감사합니다! 즐거운 하루 되십시요!");
      indexPage();
  ...
  fragment
    .querySelector(".product-billing__myPage-btn")
    .addEventListener("click", e => {
      myPages();
    });
  ...
  res.data.forEach(bill => {
    const itemFragment = document.importNode(
      templates.productBillingItem,
      true
    );
    itemFragment.querySelector(".product-billing-item__img").src =
      bill.product.imgUrl;
    itemFragment.querySelector(".product-billing-item__title").textContent =
      bill.product.title;
    itemFragment.querySelector(
        ".product-billing-item__product-count"
      ).textContent =
      bill.amount + "개";
    itemFragment.querySelector(
        ".product-billing-item__product-cost"
      ).textContent =
      commas(bill.product.cost * bill.amount) + "원";
  ...
  fragment
    .querySelector(".product-billing-form")
    .addEventListener("submit", async e => {
      e.preventDefault();
      if (confirm("결제를 진행하시겠습니까?")) {
        try {
          // 장바구니 비워버리기 && 수량과 보유 금액 차감 기능 구현 - 추가로 하기
          // const resCart = await productAPI.get('carts')
...
async function productPage(productId) {
  ...
  fragment
    .querySelector(".product-content__count-plus")
    .addEventListener("click", () => {
      if (orderCount.value < res.data.productCount) {
        count++;
        orderCount.value = count;
        orderCost.textContent = `결제 예상 금액 ${commas(
          orderCount.value * cost
        )}원`;
      }
    });
  fragment
    .querySelector(".product-content__count-minus")
    .addEventListener("click", () => {
  ...
  fragment.querySelector(".product-content__title").textContent =
    res.data.title;
  fragment.querySelector(
    ".product-content__cost"
  ).textContent = `결제 예상 금액 ${commas(cost)}원`;
  ...
  fragment
    .querySelector(".product-content__cart-btn")
    .addEventListener("click", async e => {
      e.preventDefault();
  ...
  fragment
    .querySelector(".product-content__order-btn")
    .addEventListener("click", async e => {
      e.preventDefault();
      if (localStorage.getItem("token")) {
        const payload = {
  ...
  fragment
    .querySelector(".product__myPage-btn")
    .addEventListener("click", e => {
      myPages();
    });

  fragment
    .querySelector(".product-content__back-btn")
    .addEventListener("click", e => {
      indexPage();
    });
  ...
  fragment
    .querySelector(".product-content__edit-btn")
    .addEventListener("click", () => {
      editFormPage(productId);
    });
  fragment
    .querySelector(".product-content__delete-btn")
    .addEventListener("click", async e => {
  ...
  if (localStorage.getItem("token")) {
    ...
    const commentsRes = await productAPI.get(
      `products/${productId}/comments?_expand=user`
    );
    commentsRes.data.forEach(comment => {
      ...
      const removeButtonEl = itemFragment.querySelector(
        ".comment-item__remove-btn"
      );
      authorEl.textContent = comment.user.username + " :";
      bodyEl.textContent = comment.body;
      commentsFragment
        .querySelector(".comments__list")
        .appendChild(itemFragment);
      removeButtonEl.addEventListener("click", async e => {
        const res = await productAPI.delete(`comments/${comment.id}`).then(
          response => {
            authorEl.remove();
            bodyEl.remove();
...
async function indexPage() {
  ...
  listFragment
    .querySelector(".product__post-btn")
    .addEventListener("click", e => {
      postProductPage();
    });
  ...
  listFragment
    .querySelector(".product__login-btn")
    .addEventListener("click", e => {
      loginPage();
    });
  listFragment
  ...
  listFragment
    .querySelector(".product__join-btn")
    .addEventListener("click", e => {
      joinPage();
    });
